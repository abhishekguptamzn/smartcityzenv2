...
class _FacilityManualCheckinScreenState
  ...
  Future<void> _handleBulkCheckin(
    ...
    final confirm = await showAppConfirmDialog(
      ...
      title: 'Bulk Check-In',
      message:
          'Are you sure you want to log desk check-in for ${outsideSelected.length} member(s)?',
      confirmLabel: 'Check In ${outsideSelected.length} Members',
      details: [
        ConfirmDetailRow(
            label: 'Members to Check In', value: '${outsideSelected.length}'),
        ConfirmDetailRow(
  ...
  Future<void> _handleBulkCheckout(
    ...
    if (insideSelected.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('None of the selected member(s) are currently inside.'),
          backgroundColor: Color(0xFFD97706),
          behavior: SnackBarBehavior.floating,
        ),
      );
    ...
    final confirm = await showAppConfirmDialog(
      ...
      message:
          'Are you sure you want to check out ${insideSelected.length} member(s)?',
      confirmLabel: 'Check Out ${insideSelected.length} Members',
      type: ConfirmDialogType.warning,
      details: [
        ConfirmDetailRow(
            label: 'Members to Check Out', value: '${insideSelected.length}'),
        ConfirmDetailRow(
  ...
  void _openQrScanner() {
    CitizenManualCheckinQrModal.show(
      context: context,
      kind: widget.kind,
      facilityId: widget.facilityId,
      facility: widget.facility,
      onCodeScanned: (code) {
        _quickCodeController.text = code;
      },
      onScanComplete: () {
        _refreshAll();
      },
                                    ...
                                    children: [
                                      Row(
                                        ...
                                        children: [
                                          ...
                                          // Live Status Badge & Optional Renew
                                          Row(
                                            mainAxisSize: MainAxisSize.min,
                                            children: [
                                              if (isExpired || isExpiringSoon)
                                                GestureDetector(
                                                  onTap: () => _openRenewModal(member),
                                                  child: Container(
                                                    ...
                                                    child: const Row(
                                                      ...
                                                      children: [
                                                        Icon(Icons.autorenew_rounded,
                                                            size: 10, color: Color(0xFF0D9488)),
                                                        SizedBox(width: 2),
                                                        Text(
                                                          'Renew',
                                                          style: TextStyle(
